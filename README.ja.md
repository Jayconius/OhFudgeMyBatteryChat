...
すべてのデバイスとアラートで、**自分の画像・GIF・WebM動画**と専用の**サウンド**を使えます。空欄のままなら内蔵アイコンが使われます。アプリには、選べるイラスト付きのヘッドセット・コントローラー・トラッカー素材集も同梱されています（画像の2段目がそれです）。

<p align="center"><img src="docs/images/custom-graphics.png" alt="同じオーバーレイを2つ並べた画像：内蔵アイコンのものと、イラスト付きのヘッドセット・コントローラー・トラッカーを自分の画像として選んだもの" width="100%"></p>

1. **デバイスを追加**を開きます（一覧のデバイスをダブルクリックして編集してもかまいません）。
2. **メディア**欄の**通常時の画像:**の横の**選択...**をクリックします。同梱の素材集が開くので、そこから選ぶか、自分のファイルを指定します。
3. バッテリーが少なくなったときに画像を変えたい場合は**低下時の画像:**も、アラート音には**警告音:**も選びます。
4. **保存**をクリックします。OBSのオーバーレイは自動で更新されます。

アラート（**オーバーレイ効果を追加**）も同じです。**画像:**とサウンドを選びます。 素材集がなくなった場合は、**ℹ 情報 > アイコンを復元**で元に戻せます。
