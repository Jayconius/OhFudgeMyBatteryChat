...
STRINGS = {
    ...
    "msg_tool_missing_fmt": {"en": "{product} ({name}) isn't next to this program yet.\n\nDownload it from GitHub now? It's about {mb} MB, saved in this program's folder, and checked against the checksum GitHub publishes for it. Nothing else is downloaded or installed.", "de": "{product} ({name}) liegt noch nicht neben diesem Programm.\n\nJetzt von GitHub herunterladen? Etwa {mb} MB, wird im Ordner dieses Programms gespeichert und gegen die von GitHub veröffentlichte Prüfsumme geprüft. Sonst wird nichts heruntergeladen oder installiert.", "fr": "{product} ({name}) n'est pas encore à côté de ce programme.\n\nLe télécharger depuis GitHub maintenant ? Environ {mb} Mo, enregistré dans le dossier de ce programme et vérifié avec la somme de contrôle publiée par GitHub. Rien d'autre n'est téléchargé ni installé.", "es": "{product} ({name}) todavía no está junto a este programa.\n\n¿Descargarlo ahora desde GitHub? Son unos {mb} MB, se guarda en la carpeta de este programa y se comprueba con la suma de verificación que publica GitHub. No se descarga ni instala nada más.", "ja": "{product}（{name}）がこのプログラムの隣にまだありません。\n\nGitHubから今すぐダウンロードしますか？約{mb}MBで、このプログラムのフォルダーに保存され、GitHubが公開しているチェックサムで検証されます。それ以外はダウンロードもインストールもされません。"},
    "msg_tool_downloading_fmt": {"en": "Downloading {product}...", "de": "{product} wird heruntergeladen...", "fr": "Téléchargement de {product}...", "es": "Descargando {product}...", "ja": "{product}をダウンロード中..."},
    "msg_tool_launch_failed_fmt": {"en": "Couldn't start {product}: {error}", "de": "{product} konnte nicht gestartet werden: {error}", "fr": "Impossible de démarrer {product} : {error}", "es": "No se pudo iniciar {product}: {error}", "ja": "{product}を起動できませんでした: {error}"},
    "companion_err_no_release": {"en": "{product} hasn't been published on GitHub yet.", "de": "{product} wurde auf GitHub noch nicht veröffentlicht.", "fr": "{product} n'a pas encore été publié sur GitHub.", "es": "{product} todavía no se ha publicado en GitHub.", "ja": "{product}はまだGitHubで公開されていません。"},
    "btn_open_simulator": {"en": "Open Demo Simulator", "de": "Demo-Simulator öffnen", "fr": "Ouvrir le Simulateur de démo", "es": "Abrir Simulador de demostración", "ja": "デモシミュレーターを開く"},
    "hint_simulator": {"en": "Try your setup without a headset: it feeds the app fake devices and microphones. While it's running the app shows those instead of your real ones - close it to go back.", "de": "Teste deine Einrichtung ohne Headset: Er liefert der App gefälschte Geräte und Mikrofone. Solange er läuft, zeigt die App diese statt deiner echten - schließe ihn, um zurückzukehren.", "fr": "Testez votre configuration sans casque : il fournit à l'application de faux appareils et micros. Tant qu'il tourne, l'application les affiche à la place des vrais - fermez-le pour revenir.", "es": "Prueba tu configuración sin casco: le da a la aplicación dispositivos y micrófonos falsos. Mientras esté abierto, la aplicación muestra esos en lugar de los reales; ciérralo para volver.", "ja": "ヘッドセットなしで設定を試せます。アプリに偽のデバイスとマイクを送ります。実行中はアプリが本物の代わりにそれらを表示します。元に戻すには閉じてください。"},
